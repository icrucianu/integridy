 ...
 */
@Stateless
public class CumulClientWFacade extends AbstractFacade<CumulConsumerW> {
    ...
    public CumulClientWFacade() {
        super(CumulConsumerW.class);
    }
    public List<CumulConsumerW> getCumulWbyUsernameAndDate(String userName , Date date){
        ...
        String sqlString = "select rec.* \n" +
                            ...
                            "       integridyschema.cumul_consumer_w rec,\n" +
        ...
        Query query = em.createNativeQuery(sqlString, CumulConsumerW.class);
     ...
     public List<Object[]> getTotalWeeklyConsumptionByUsername(String username, Date date){
      ...
      @Override
     public List<CumulConsumerW> getByUsernameRefDateAndConsPoint(String username, Date refDate, int consumptionPointId){
        ...
        List<CumulConsumerW> resultList = query.getResultList();
        return resultList;
    }
     ...
     public List<Object[]> getTotalConsumptionByClient(int clientId , Date refDate){
        ...
        Calendar calendarStart = Calendar.getInstance();
        calendarStart.setTime(refDate);
        calendarStart.add(Calendar.DAY_OF_YEAR, -200);
        Date dateStart = calendarStart.getTime();
        
        Calendar calendarStop = Calendar.getInstance();
        calendarStop.setTime(refDate);
        calendarStop.add(Calendar.DAY_OF_YEAR, 200);
        Date dateStop = calendarStop.getTime();
      
        String sqlString = "select sum(coalesce(rec.consumed_active_pow,0)), rec.created_time\n" +
                            "from integridyschema.cumul_consumer_w rec,\n" +
    ...
    @Override
    public List<CumulConsumerW> getByClientAndRefDate(int clientId, Date refDate) {
       SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
         List<CumulConsumerW>  ret= null;
         // +/- 30 zile reference time
        Calendar calendarStart = Calendar.getInstance();
        calendarStart.setTime(refDate);
        calendarStart.add(Calendar.DAY_OF_YEAR, -200);
        ...
        String sqlString = "select rec.* \n" +
                            ...
                            "		and date(rec.created_time) between to_date(?2,'yyyy-MM-dd') and to_date(?3,'yyyy-MM-dd') " +
                            "order by rec.created_time";
         Query query = getEntityManager().createNativeQuery(sqlString,CumulConsumerW.class);
         query.setParameter(1, clientId);
         query.setParameter(2, df.format(dateStart));
